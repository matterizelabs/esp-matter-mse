...
#define SLOT_SECTORS (ANIM_SLOT_SIZE / SECTOR_SIZE)
...
static uint32_t s_sector_erased[ANIM_SLOT_COUNT];
...
static uint8_t s_busy[ANIM_SLOT_COUNT];

/* all access is serialized by the single animation io task; no locking required */
...
esp_err_t anim_flash_abort(int slot) {
    if (slot < 0 || slot >= ANIM_SLOT_COUNT) return ESP_ERR_INVALID_ARG;
    s_busy[slot] = 0;
...
esp_err_t anim_flash_write(int slot, uint32_t offset, const uint8_t *data, size_t len) {
    if (slot < 0 || slot >= ANIM_SLOT_COUNT || !data || len == 0) return ESP_ERR_INVALID_ARG;
    if ((uint64_t)offset + len > ANIM_SLOT_SIZE) return ESP_ERR_INVALID_ARG;
    uint32_t base = DATA_OFFSET + (uint32_t)slot * ANIM_SLOT_SIZE;
...
esp_err_t anim_flash_read_frame(int slot, uint32_t frame_index, uint8_t *out, size_t out_len) {
    if (slot < 0 || slot >= ANIM_SLOT_COUNT || !out || out_len < FRAME_BYTES) return ESP_ERR_INVALID_SIZE;
    if ((uint64_t)(frame_index + 1) * FRAME_BYTES > ANIM_SLOT_SIZE) return ESP_ERR_INVALID_ARG;
    return esp_partition_read(s_part, DATA_OFFSET + (uint32_t)slot * ANIM_SLOT_SIZE + frame_index * FRAME_BYTES, out,
                              FRAME_BYTES);
}

esp_err_t anim_flash_get_slot_info(int slot, uint16_t *total_frames, uint8_t *fps, uint8_t *loop) {
    if (slot < 0 || slot >= ANIM_SLOT_COUNT || s_busy[slot] || s_meta.slots[slot].state != 1) return ESP_ERR_INVALID_STATE;
...
esp_err_t anim_flash_get_slot_hash(int slot, uint8_t *out, size_t len) {
    if (slot < 0 || slot >= ANIM_SLOT_COUNT || !out || len < 32 || s_busy[slot] || s_meta.slots[slot].state != 1)
